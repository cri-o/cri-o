...
import (
	...
	internalStorage "github.com/containers/image/v5/internal/storage"
...
func (ic *imageCopier) copyLayer(ctx context.Context, srcInfo types.BlobInfo, toEncrypt bool, pool *mpb.Progress, layerIndex int) (types.BlobInfo, digest.Digest, error) {
	...
	if !diffIDIsNeeded {
		...
		storageDest, ok := ic.c.dest.(*internalStorage.StorageImageDestination)
		if ok {
			options := internalStorage.TryReusingBlobOptions{
				Cache:         ic.c.blobInfoCache,
				CanSubstitute: ic.canSubstituteBlobs,
				LayerIndex:    &layerIndex,
			}
			reused, blobInfo, err = storageDest.TryReusingBlobWithOptions(ctx, srcInfo, options)
		} else {
...
func (c *copier) copyBlobFromStream(ctx context.Context, srcStream io.Reader, srcInfo types.BlobInfo,
	...
	storageDest, ok := c.dest.(*internalStorage.StorageImageDestination)
	if ok {
		options := internalStorage.PutBlobOptions{
		...
		uploadedInfo, err = storageDest.PutBlobWithOptions(ctx, &errorAnnotationReader{destStream}, inputInfo, options)
	} else {
