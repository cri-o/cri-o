...
type connectionContext struct {
	...
	resizeChan   chan resize.TerminalSize
	tty          bool
}
...
func createStreams(req *http.Request, w http.ResponseWriter, opts *Options, supportedStreamProtocols []string, idleTimeout, streamCreationTimeout time.Duration) (*connectionContext, bool) {
	...
	if ctx.resizeStream != nil {
		ctx.resizeChan = make(chan resize.TerminalSize)
	...
	for {
		...
		case stream := <-streams:
			...
			case api.StreamTypeError:
				...
				defer stream.Reset() //nolint: gocritic,errcheck
...
func handleResizeEvents(stream io.Reader, channel chan<- resize.TerminalSize) {
