...
func (s *Server) StopPodSandbox(
	ctx context.Context,
	req *types.StopPodSandboxRequest,
) (*types.StopPodSandboxResponse, error) {
	...
	if err != nil {
		...
		if errors.Is(err, errSandboxNotCreated) {
			return nil, fmt.Errorf(
				"StopPodSandbox failed as the sandbox is not created: %s",
				req.GetPodSandboxId(),
			)
		}
		...
		log.Warnf(
			ctx,
			"Could not get sandbox %s, it's probably been stopped already: %v",
			req.GetPodSandboxId(),
			err,
		)
