...
func TestConfigureGeneratorForSysctlsUnprivilegedPort(t *testing.T) {
	...
	cases := []struct {
		...
		{
			description:  "sets default when opted in and pod has its own network namespace",
			optedIn:      true,
			hostNetwork:  false,
			inputSysctls: map[string]string{},
			wantValue:    "0",
		...
		{
			description:  "does not set when not opted in",
			optedIn:      false,
			hostNetwork:  false,
			inputSysctls: map[string]string{},
			wantAbsent:   true,
			...
			s := &Server{
				config: libconfig.Config{
					RuntimeConfig: libconfig.RuntimeConfig{
						DefaultUnprivilegedPortStart: tc.optedIn,
					},
				},
			}
