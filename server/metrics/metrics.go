...
func (m *Metrics) startEndpoint(
	...
	go func() {
		...
		if m.config.MetricsCert != "" && m.config.MetricsKey != "" {
			...
			if err = cert.GenerateSelfSignedCertKey(m.config.MetricsCert, m.config.MetricsKey); err != nil {
				log.Fatalf(ctx, "Generating self-signed cert/key: %v", err)
			}

			var cc *cert.CertConfig
			cc, err = cert.NewCertConfig(stop, m.config.MetricsCert, m.config.MetricsKey, "")
			if err != nil {
