...
import (
	"errors"
	"fmt"
	"github.com/cri-o/cri-o/internal/lib/sandbox/builder"
	"os"
	"path/filepath"
	"strconv"
	"strings"
...
func (s *Server) runPodSandbox(ctx context.Context, req *types.RunPodSandboxRequest) (resp *types.RunPodSandboxResponse, retErr error) {
	...
	sbuilder := builder.NewSandboxBuilder()
	if err := sbuilder.SetConfig(req.Config); err != nil {
		return nil, fmt.Errorf("setting sandbox config: %w", err)
	}

	sbuilderConfig := sbuilder.Config()

	kubeName := sbuilderConfig.Metadata.Name
	kubePodUID := sbuilderConfig.Metadata.Uid
	namespace := sbuilderConfig.Metadata.Namespace
	attempt := sbuilderConfig.Metadata.Attempt
	...
	log.Infof(ctx, "Running pod sandbox: %s%s", translateLabelsToDescription(sbuilder.Config().Labels), oci.InfraContainerName)

	if err := sbuilder.SetName(); err != nil {
		return nil, fmt.Errorf("setting pod sandbox name: %w", err)
	}
	if err := sbuilder.SetID(); err != nil {
		return nil, fmt.Errorf("setting pod sandbox id: %w", err)
	}
	...
	sbname, err := sbuilder.SandboxName()
	if err != nil {
		return nil, fmt.Errorf("getting pod sandbox name: %w", err)
	}

	sbid, err := sbuilder.SandboxID()
	if err != nil {
		return nil, fmt.Errorf("getting pod sandbox id: %w", err)
	}

	if _, err := s.ReservePodName(sbid, sbname); err != nil {
		reservedID, getErr := s.PodIDForName(sbname)
		if getErr != nil {
			return nil, fmt.Errorf("failed to get ID of pod with reserved name (%s), after failing to reserve name with %w: %w", sbname, getErr, getErr)
		}
		...
		cachedID, resourceErr := s.getResourceOrWait(ctx, sbname, "sandbox")
		if resourceErr == nil {
			return &types.RunPodSandboxResponse{PodSandboxId: cachedID}, nil
		}
		return nil, fmt.Errorf("%w: %w", resourceErr, err)
	}
	resourceCleaner.Add(ctx, "runSandbox: releasing pod sandbox name: "+sbname, func() error {
		s.ReleasePodName(sbname)
		return nil
	})

	s.resourceStore.SetStageForResource(ctx, sbname, "sandbox creating")
	...
	if err := s.config.CNIPluginReadyOrError(); err != nil && !hostNetwork {
		...
		log.Infof(ctx, "CNI plugin not ready. Waiting to create %s as it is not host network", sbname)
		if ready := <-watcher; !ready {
			return nil, fmt.Errorf("server shutdown before network was ready: %w", err)
		}
		log.Infof(ctx, "CNI plugin is now ready. Continuing to create %s", sbname)
	}
	s.resourceStore.SetStageForResource(ctx, sbname, "sandbox network ready")
	...
	s.resourceStore.SetStageForResource(ctx, sbname, "sandbox storage creation")
	...
	if errors.Is(err, storage.ErrDuplicateName) {
		return nil, fmt.Errorf("pod sandbox with name %q already exists", sbname)
	}
	if err != nil {
		return nil, fmt.Errorf("creating pod sandbox with name %q: %w", sbname, err)
	}
	resourceCleaner.Add(ctx, "runSandbox: removing pod sandbox from storage: "+sbid, func() error {
		return s.StorageRuntimeServer().DeleteContainer(ctx, sbid)
	})
	...
	if logDir == "" {
		logDir = filepath.Join(s.config.LogDir, sbid)
	}
	// This should always be absolute from k8s.
	if !filepath.IsAbs(logDir) {
		return nil, fmt.Errorf("requested logDir for sbox ID %s is a relative path: %s", sbid, logDir)
	}
	...
	s.resourceStore.SetStageForResource(ctx, sbname, "sandbox shm creation")
	var shmPath string
	if hostIPC {
		...
		resourceCleaner.Add(ctx, "runSandbox: unmounting shmPath for sandbox "+sbid, func() error {
	...
	s.resourceStore.SetStageForResource(ctx, sbname, "sandbox spec configuration")
	...
	err = s.setPodSandboxMountLabel(ctx, sbid, mountLabel)
	if err != nil {
		return nil, err
	}

	if err := s.CtrIDIndex().Add(sbid); err != nil {
		return nil, err
	}
	resourceCleaner.Add(ctx, "runSandbox: deleting container ID from idIndex for sandbox "+sbid, func() error {
		if err := s.CtrIDIndex().Delete(sbid); err != nil && !strings.Contains(err.Error(), noSuchID) {
			return fmt.Errorf("could not delete ctr id %s from idIndex: %w", sbid, err)
		}
		return nil
	})

	// set log path inside log directory
	logPath := filepath.Join(logDir, sbid+".log")
	...
	hostname, err := getHostname(sbid, sbuilder.Config().Hostname, hostNetwork)
	if err != nil {
		return nil, err
	}

	resolvPath, err := sbuilder.ResolvPath()
	if err != nil {
		errors.New("Resolve path cannot be determined or not set")
	}
	...
	g.AddAnnotation(annotations.Name, sbname)
	g.AddAnnotation(annotations.SandboxName, sbname)
	g.AddAnnotation(annotations.Namespace, namespace)
	g.AddAnnotation(annotations.ContainerType, annotations.ContainerTypeSandbox)
	g.AddAnnotation(annotations.SandboxID, sbid)
	...
	g.AddAnnotation(annotations.ContainerID, sbid)
	...
	cgroupParent, cgroupPath, err := s.config.CgroupManager().SandboxCgroupPath(sbuilder.Config().Linux.CgroupParent, sbid, containerMinMemory)
	if err != nil {
		return nil, err
	}
	if cgroupPath != "" {
	...
	//sb, err := libsandbox.New(sbid, namespace, sbname, kubeName, logDir, labels, kubeAnnotations, processLabel, mountLabel, metadata, shmPath, cgroupParent, privileged, runtimeHandler, resolvPath, hostname, portMappings, hostNetwork, created, usernsMode, overhead, resources)
	//if err != nil {
	//	return nil, err
	//}
	...
	err = sbuilder.SetCriSandbox(created, labels, kubeAnnotations, metadata)
	if err != nil {
		return nil, err
	}
	sbuilder.SetContainers()
	...
	//sb.SetDNSConfig(sbuilder.Config().DnsConfig)

	if err := s.addSandbox(ctx, sb); err != nil {
		return nil, err
	}
	resourceCleaner.Add(ctx, "runSandbox: removing pod sandbox "+sbid, func() error {
		if err := s.removeSandbox(ctx, sbid); err != nil {
			return fmt.Errorf("could not remove pod sandbox: %w", err)
		}
		return nil
	})

	if err := s.PodIDIndex().Add(sbid); err != nil {
		return nil, err
	}
	resourceCleaner.Add(ctx, "runSandbox: deleting pod ID "+sbid+" from idIndex", func() error {
		if err := s.PodIDIndex().Delete(sbid); err != nil && !strings.Contains(err.Error(), noSuchID) {
			return fmt.Errorf("could not delete pod id %s from idIndex: %w", sbid, err)
		}
		return nil
	})
	...
	// set up namespaces
	s.resourceStore.SetStageForResource(ctx, sbname, "sandbox namespace creation")
	...
	nsCleanupDescription := "runSandbox: cleaning up namespaces after failing to run sandbox " + sbid
	...
	s.resourceStore.SetStageForResource(ctx, sbname, "sandbox network creation")
	...
	s.resourceStore.SetStageForResource(ctx, sbname, "sandbox storage start")

	mountPoint, err := s.StorageRuntimeServer().StartContainer(sbid)
	if err != nil {
		return nil, fmt.Errorf("failed to mount container %s in pod sandbox %s(%s): %w", containerName, sb.Name(), sbid, err)
	}
	resourceCleaner.Add(ctx, "runSandbox: stopping storage container for sandbox "+sbid, func() error {
		if err := s.StorageRuntimeServer().StopContainer(ctx, sbid); err != nil {
			return fmt.Errorf("could not stop storage container: %s: %w", sbid, err)
		}
		return nil
	})
	...
	if sb.NeedsInfra(s.config.DropInfraCtr) || podIsKernelSeparated {
		log.Debugf(ctx, "Keeping infra container for pod %s", sbid)
		// pauseImage, as the userRequestedImage parameter, only shows up in CRI values we return.
		container, err = oci.NewContainer(sbid, containerName, podContainer.RunDir, logPath, labels, g.Config.Annotations, kubeAnnotations, pauseImage.StringForOutOfProcessConsumptionOnly(), nil, nil, "", nil, sbid, false, false, false, runtimeHandler, podContainer.Dir, created, podContainer.Config.Config.StopSignal)
		if err != nil {
			return nil, err
		}
		...
		log.Debugf(ctx, "Dropping infra container for pod %s", sbid)
		container = oci.NewSpoofedContainer(sbid, containerName, labels, sbid, created, podContainer.RunDir)
		g.AddAnnotation(annotations.SpoofedContainer, "true")
		if err := s.config.CgroupManager().CreateSandboxCgroup(cgroupParent, sbid); err != nil {
			return nil, fmt.Errorf("create dropped infra %s cgroup: %w", sbid, err)
		}
	}
	...
	if err = g.SaveToFile(filepath.Join(podContainer.Dir, "config.json"), saveOptions); err != nil {
		return nil, fmt.Errorf("failed to save template configuration for pod sandbox %s(%s): %w", sb.Name(), sbid, err)
	}
	if err = g.SaveToFile(filepath.Join(podContainer.RunDir, "config.json"), saveOptions); err != nil {
		return nil, fmt.Errorf("failed to write runtime configuration for pod sandbox %s(%s): %w", sb.Name(), sbid, err)
	}
	...
	s.resourceStore.SetStageForResource(ctx, sbname, "sandbox container runtime creation")
	...
	if isContextError(ctx.Err()) {
		if err := s.resourceStore.Put(sbname, sb, resourceCleaner); err != nil {
			log.Errorf(ctx, "RunSandbox: failed to save progress of sandbox %s: %v", sbid, err)
		}
	...
	resp = &types.RunPodSandboxResponse{PodSandboxId: sbid}
	return resp, nil
}
