...
import (
	...
	sbuilderfactory "github.com/cri-o/cri-o/internal/factory/sandbox"
...
func (s *Server) runPodSandbox(ctx context.Context, req *types.RunPodSandboxRequest) (resp *types.RunPodSandboxResponse, retErr error) {
	...
	if _, err := s.ReservePodName(sbuilder.ID(), sbuilder.Name()); err != nil {
		reservedID, getErr := s.PodIDForName(sbuilder.Name())
		if getErr != nil {
			return nil, fmt.Errorf("failed to get ID of pod with reserved name (%s), after failing to reserve name with %w: %w", sbuilder.Name(), getErr, getErr)
		}
		...
		cachedID, resourceErr := s.getResourceOrWait(ctx, sbuilder.Name(), "sandbox")
		if resourceErr == nil {
			return &types.RunPodSandboxResponse{PodSandboxId: cachedID}, nil
		}
		return nil, fmt.Errorf("%w: %w", resourceErr, err)
	}
	resourceCleaner.Add(ctx, "runSandbox: releasing pod sandbox name: "+sbuilder.Name(), func() error {
		s.ReleasePodName(sbuilder.Name())
		return nil
	})

	s.resourceStore.SetStageForResource(ctx, sbuilder.Name(), "sandbox creating")
	...
	if err := s.config.CNIPluginReadyOrError(); err != nil && !hostNetwork {
		...
		log.Infof(ctx, "CNI plugin not ready. Waiting to create %s as it is not host network", sbuilder.Name())
		if ready := <-watcher; !ready {
			return nil, fmt.Errorf("server shutdown before network was ready: %w", err)
		}
		log.Infof(ctx, "CNI plugin is now ready. Continuing to create %s", sbuilder.Name())
	}
	s.resourceStore.SetStageForResource(ctx, sbuilder.Name(), "sandbox network ready")
	...
	if errors.Is(err, storage.ErrDuplicateName) {
		return nil, fmt.Errorf("pod sandbox with name %q already exists", sbuilder.Name())
	}
	if err != nil {
		return nil, fmt.Errorf("creating pod sandbox with name %q: %w", sbuilder.Name(), err)
	}
	resourceCleaner.Add(ctx, "runSandbox: removing pod sandbox from storage: "+sbuilder.ID(), func() error {
		return s.StorageRuntimeServer().DeleteContainer(ctx, sbuilder.ID())
	})
	...
	if logDir == "" {
		logDir = filepath.Join(s.config.LogDir, sbuilder.ID())
	}
	// This should always be absolute from k8s.
	if !filepath.IsAbs(logDir) {
		return nil, fmt.Errorf("requested logDir for sbuilder ID %s is a relative path: %s", sbuilder.ID(), logDir)
	}
	...
	s.resourceStore.SetStageForResource(ctx, sbuilder.Name(), "sandbox shm creation")
	var shmPath string
	if hostIPC {
		...
		shmPath, err = sbuilderfactory.SetupShm(podContainer.RunDir, mountLabel, shmSize)
		if err != nil {
			return nil, err
		}
		...
		resourceCleaner.Add(ctx, "runSandbox: unmounting shmPath for sandbox "+sbuilder.ID(), func() error {
	...
	s.resourceStore.SetStageForResource(ctx, sbuilder.Name(), "sandbox spec configuration")
	...
	err = s.setPodSandboxMountLabel(ctx, sbuilder.ID(), mountLabel)
	if err != nil {
		return nil, err
	}

	if err := s.CtrIDIndex().Add(sbuilder.ID()); err != nil {
		return nil, err
	}
	resourceCleaner.Add(ctx, "runSandbox: deleting container ID from idIndex for sandbox "+sbuilder.ID(), func() error {
		if err := s.CtrIDIndex().Delete(sbuilder.ID()); err != nil && !strings.Contains(err.Error(), noSuchID) {
			return fmt.Errorf("could not delete ctr id %s from idIndex: %w", sbuilder.ID(), err)
		}
		return nil
	})

	// set log path inside log directory
	logPath := filepath.Join(logDir, sbuilder.ID()+".log")
	...
	hostname, err := getHostname(sbuilder.ID(), sbuilder.Config().Hostname, hostNetwork)
	if err != nil {
		return nil, err
	}
	...
	g.AddAnnotation(annotations.Name, sbuilder.Name())
	g.AddAnnotation(annotations.SandboxName, sbuilder.Name())
	g.AddAnnotation(annotations.Namespace, namespace)
	g.AddAnnotation(annotations.ContainerType, annotations.ContainerTypeSandbox)
	g.AddAnnotation(annotations.SandboxID, sbuilder.ID())
	...
	g.AddAnnotation(annotations.ContainerID, sbuilder.ID())
	...
	cgroupParent, cgroupPath, err := s.config.CgroupManager().SandboxCgroupPath(sbuilder.Config().Linux.CgroupParent, sbuilder.ID(), containerMinMemory)
	if err != nil {
		return nil, err
	}
	if cgroupPath != "" {
	...
	//sb, err := libsandbox.New(sbuilder.ID(), namespace, sbuilder.Name(), kubeName, logDir, labels, kubeAnnotations, processLabel, mountLabel, metadata, shmPath, cgroupParent, privileged, runtimeHandler, sbuilder.ResolvPath(), hostname, portMappings, hostNetwork, created, usernsMode, overhead, resources)
	//if err != nil {
	//	return nil, err
	//}
