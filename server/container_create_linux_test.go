...
func TestAddOCIBindsForDev(t *testing.T) {
	...
	_, binds, err := ctr.AddOCIBindMounts(context.Background(), "", "", nil, false, false, false, false, "")
	if err != nil {
		t.Error(err)
	}
...
func TestAddOCIBindsCGroupRW(t *testing.T) {
	...
	_, _, err = ctr.AddOCIBindMounts(context.Background(), "", "", nil, false, false, true, false, "")
	if err != nil {
		t.Error(err)
	}
	...
	_, _, err = ctr.AddOCIBindMounts(context.Background(), "", "", nil, false, false, false, false, "")
	if err != nil {
		t.Error(err)
	}
	for _, m := range ctr.Spec().Mounts() {
...
func TestAddOCIBindsErrorWithoutIDMap(t *testing.T) {
	...
	_, _, err = ctr.AddOCIBindMounts(context.Background(), "", "", nil, false, false, false, true, "")
	if err != nil {
		t.Errorf("%v", err)
	}
}
