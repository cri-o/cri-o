...
func New(
	...
	if config.StreamEnableTLS {
		logrus.Debugf("TLS enabled for streaming server")
		certConf, err := cert.NewCertConfig(s.stream.streamServerCloseCh, config.StreamTLSCert, config.StreamTLSKey, config.StreamTLSCA)
		if err != nil {
			return nil, err
		}
