...
func (s *Server) runPodSandbox(ctx context.Context, req *types.RunPodSandboxRequest) (resp *types.RunPodSandboxResponse, retErr error) {
	...
	if _, err := s.ReservePodName(sandboxId, sandboxName); err != nil {
		...
		if getErr != nil {
			return nil, fmt.Errorf("failed to get ID of pod with reserved name (%s), after failing to reserve name with %v: %w", sandboxName, getErr, getErr)
		}
	...
	if sbuilder.Config().Linux != nil && sbuilder.Config().Linux.SecurityContext != nil {
	...
	if err := sbuilder.InitInfraContainer(&s.config, &podContainer, nil); err != nil {
		return nil, err
	}
	...
	//sb, err := libsandbox.New(sandboxId, namespace, sandboxName, kubeName, logDir, labels, kubeAnnotations, processLabel, mountLabel, metadata, "", "", privileged, runtimeHandler, sbuilder.ResolvPath(), hostname, portMappings, hostNetwork, created, usernsMode, overhead, resources)
	//if err != nil {
	//	return nil, err
	//}
	...
	nsCleanupDescription := fmt.Sprintf("runSandbox: cleaning up namespaces after failing to run sandbox %s", sandboxId)
