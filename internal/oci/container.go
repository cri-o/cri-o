...
func (cstate *ContainerState) SetInitPid(pid int) error {
	...
	cstate.InitPid = pid
	startTime, err := getInitStartTime(pid)
	if err != nil {
		return err
	}
...
func (c *Container) pid() (int, error) {
	if c.state == nil {
		return -1, errors.New("state not initialized")
	}
	pid := c.state.InitPid
	if pid <= 0 {
		return -1, errors.New("PID not initialized")
	}
	...
	if err := c.verifyPid(pid); err != nil {
		return -1, err
	}
	return pid, nil
}
...
func (c *Container) verifyPid(pid int) error {
	startTime, err := getInitStartTime(pid)
	if err != nil {
		return err
	}
