...
type RuntimeImpl interface {
	...
	AttachContainer(*Container, io.Reader, io.WriteCloser, io.WriteCloser,
		bool, <-chan remotecommand.TerminalSize) error
	PortForwardContainer(string, int32, io.ReadWriter) error
...
func (r *Runtime) PortForwardContainer(c *Container, nsPath string, port int32, stream io.ReadWriter) error {
	impl, err := r.RuntimeImpl(c)
	if err != nil {
		return err
	}

	return impl.PortForwardContainer(nsPath, port, stream)
}
