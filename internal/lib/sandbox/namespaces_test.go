...
var _ = t.Describe("SandboxManagedNamespaces", func() {
	...
	t.Describe("*Ns", func() {
		...
		It("should get something when network is set", func() {
			...
			_, err := testSandbox.CreateNamespacesWithFunc(managedNamespaces, "", getSelf.pinNamespaces)
			Expect(err).To(BeNil())
