...
func (b *builder) InitInfraContainer(serverConfig *libconfig.Config, podContainer *storage.ContainerInfo, sandboxIDMappings *idtools.IDMappings) error {
	var err error
...
func (b *builder) Spec() *generate.Generator {
	return b.infra.Spec()
}
...
func (b *builder) createResolvConf(podContainer *storage.ContainerInfo, sandboxIDMappings *idtools.IDMappings) (retErr error) {
	// set DNS options
	b.sb.resolvPath = podContainer.RunDir + "/resolv.conf"
	...
	err := ParseDNSOptions(dnsServers, dnsSearches, dnsOptions, b.sb.resolvPath)
	defer func() {
		if retErr != nil {
			if err := os.Remove(b.sb.resolvPath); err != nil {
	...
	if err := label.Relabel(b.sb.resolvPath, podContainer.MountLabel, false); err != nil && !errors.Is(err, unix.ENOTSUP) {
		return err
	}
	if sandboxIDMappings != nil {
		rootPair := sandboxIDMappings.RootPair()
		if err := os.Chown(b.sb.resolvPath, rootPair.UID, rootPair.GID); err != nil {
			return fmt.Errorf("cannot chown %s to %d:%d: %w", b.sb.resolvPath, rootPair.UID, rootPair.GID, err)
		}
	}
	mnt := spec.Mount{
		Type:        "bind",
		Source:      b.sb.resolvPath,
