...
// different ImageService when image management must be done by the runtimme.
type runtimePulledRuntimeService struct {
	...
	// Pointer to a runtimeService instance that uses a specific image service
...
func GetRuntimePulledRuntimeService(ctx context.Context, runtimeService RuntimeServer, storageImageServer ImageServer, storageTransport StorageTransport) RuntimeServer {
	...
	rs_rp := GetRuntimeService(ctx, storageImageServer, storageTransport)

	return &runtimePulledRuntimeService{
		runtimeServiceOCI:  runtimeService,
		runtimeServiceRP:   rs_rp,
