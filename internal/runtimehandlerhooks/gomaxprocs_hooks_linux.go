...
func (g *GomaxprocsHooks) PreCreate(ctx context.Context, specgen *generate.Generator, s *sandbox.Sandbox, c *oci.Container) error {
	...
	injectGOMAXPROCS(specgen, g.defaultEnv, maxProcs)
...
// Injection is skipped if GOMAXPROCS is already set in defaultEnv or the OCI spec's process env.
func injectGOMAXPROCS(specgen *generate.Generator, defaultEnv []string, maxProcs int64) {
