...
func crioCheck(c *cli.Context) error {
	...
	if c.Bool("check-state-mraker") {
		if !c.Bool("repair") {
			return fmt.Errorf("--repair flag must be specified")
		}
		statemarker := c.String("state-marker")
		statefile := c.String("state-file")
		if statefile == "" {
			return fmt.Errorf("state file must be specified in state marker checking mode")
		}
		...
		if err := os.WriteFile(statefile, []byte(statemarker), 0600); err != nil {
