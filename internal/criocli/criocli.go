...
func getCrioFlags(defConf *libconfig.Config) []cli.Flag {
    ...
    than any configuration file.`, libconfig.CrioConfigPath),
		...
		&cli.StringFlag{
			...
			Usage:     fmt.Sprintf("Path to a file like ~/.docker/config.json holding credentials necessary for pulling images from secure registries (default: %q)", defConf.GlobalAuthFile),
