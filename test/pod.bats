...
@test "non-root process cannot bind to privileged port when default_unprivileged_port_start is not enabled" {
	if test -n "$CONTAINER_UID_MAPPINGS"; then
		skip "userNS enabled"
	fi
	start_crio
	...
	# because default_unprivileged_port_start is opt-in and disabled by default
	run crictl exec --sync "$ctr_id" ncat -l 80 --idle-timeout 1 --recv-only
	[ "$status" -ne 0 ]
}

@test "non-root process can bind to privileged port with default_unprivileged_port_start enabled" {
	if test -n "$CONTAINER_UID_MAPPINGS"; then
		skip "userNS enabled"
	fi
	CONTAINER_DEFAULT_UNPRIVILEGED_PORT_START=true start_crio
	...
	# because ip_unprivileged_port_start=0 was set by default_unprivileged_port_start
