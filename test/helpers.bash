...
function setup_crio() {
	...
	# Choose the cni network default
	if [[ -n "$6" ]]; then
		cninetwork="$6"
	else
		cninetwork="crio"
...
function prepare_notdefault_network_conf() {
	mkdir -p $CRIO_CNI_CONFIG
	cat >$CRIO_CNI_CONFIG/10-crio.conf <<-EOF
{
    "cniVersion": "0.3.0",
