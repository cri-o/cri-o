...
@test "allow signed image with restrictive policy on container creation (fresh pull)" {
	start_crio
	...
	[[ "$output" == *"unable to start container process"* ]]
}

@test "deny unsigned image with restrictive policy on container creation (fresh pull)" {
	start_crio
...
@test "allow signed image with restrictive policy on container creation if already pulled (by ID)" {
	start_crio
	crictl pull "$SIGNED_IMAGE"
...
@test "deny unsigned image with restrictive policy on container creation if already pulled (by ID)" {
	start_crio
	crictl pull "$UNSIGNED_IMAGE"
...
@test "allow signed image with restrictive policy on container creation if already pulled (by tag)" {
	start_crio
	crictl pull "$SIGNED_IMAGE"
	stop_crio_no_clean
...
@test "deny unsigned image with restrictive policy on container creation if already pulled (by tag)" {
	start_crio
	crictl pull "$UNSIGNED_IMAGE"
	stop_crio_no_clean
