...
@test "tls_min_version: TLS 1.3 server should accept TLS 1.3 connections" {
	generate_test_certs
	PORT=$(free_port)
...
@test "tls_min_version: TLS 1.3 server should reject TLS 1.2 connections" {
	generate_test_certs
	PORT=$(free_port)

	start_crio_with_tls "VersionTLS13" "$PORT"
	run openssl s_client -connect "127.0.0.1:$PORT" -tls1_2
	# Check for various TLS rejection messages
...
@test "tls_min_version: TLS 1.2 server should accept TLS 1.3 connections" {
	generate_test_certs
	PORT=$(free_port)
...
@test "tls_min_version: TLS 1.2 server should accept TLS 1.2 connections" {
	generate_test_certs
	PORT=$(free_port)
